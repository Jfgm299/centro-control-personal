...
def upgrade() -> None:
    op.execute("""
        DO $$ BEGIN
            IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'scheduledfrequency') THEN
                CREATE TYPE scheduledfrequency AS ENUM ('WEEKLY', 'MONTHLY', 'YEARLY', 'CUSTOM');
            END IF;
            IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'scheduledcategory') THEN
                CREATE TYPE scheduledcategory AS ENUM ('SUBSCRIPTION', 'RECURRING', 'INSTALLMENT');
            END IF;
        END $$;
    """)
