...
from ..exceptions import (
    ExerciseNotFoundError, WorkoutNotFoundError,
    WorkoutAlreadyEndedError, ExerciseNotInWorkoutError,
)
...
class ExerciseService:
    ...
    def get_exercise_by_id_with_details(self, db, workout_id, exercise_id, user_id):
        self._get_workout_for_user(db, workout_id, user_id)
        exercise = db.query(Exercise).filter(Exercise.id == exercise_id).first()
        if not exercise:
            raise ExerciseNotFoundError(exercise_id)
        ...
        return ExerciseDetailResponse(
            ...
            sets=[SetResponse(
                ...
                weight_kg=s.weight_kg, reps=s.reps, speed_kmh=s.speed_kmh,
                incline_percent=s.incline_percent, duration_seconds=s.duration_seconds,
                rpe=s.rpe, notes=s.notes, created_at=s.created_at,
